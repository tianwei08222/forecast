...
class Forecast_Companytype_Jobnum:
    ...
    def get_data(self):
        ...
        # SQL 查询语句
        sql = "select result from tb_forecast_companytype_jobnum"
        try:
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def to_sql(self):
        ...
        self.result = self.result.replace('\'','')
        cursor = self.db.cursor()
        sql = "update tb_forecast_companytype_jobnum set forecast = '%s' where id = 1"%(self.result)
        print(sql)
        try:
            cursor.execute(sql)
            self.db.commit()
...
if __name__ == "__main__":
    ...
    li.append("adf")
    print(li)
    p = Forecast_Companytype_Jobnum()
    p.get_data()
    p.train()

{"date":['02-01','02-02'],"foreign":[180,200],"joint_stock":[125,130],"private_limited":[140,143],"nationalized_business":[150,151]}
