...
if __name__ == '__main__':
    ...
    cursor = con.cursor()
    sql = "delete from tb_forecast_companytype_salary"
    try:
        cursor.execute(sql)
        con.commit()
    except:
    ...
    for dir in range(1, 10):
        ...
        for date_num in range(29, -1, -1):
            ...
            forecast = forecast_companytype_salary(dir, today)
            final_results = forecast.search(today)
            dic = {}
            dic['date'] = str(today)
            # 各公司类型的薪资 外资、合资、民营、国企 （薪资）
            foreign_salary = []
            joint_stock_salary = []
            private_limited_salary = []
            nationalized_business_salary = []
                ...
                if it['company_type'] is not None and len(it['company_type']) > 0:
                    if "外资" in it['company_type']:
                        if it['job_salary_min'] is not None and len(it['job_salary_min']) > 0 and it['job_salary_max'] is not None and len(it['job_salary_max']) > 0:
                            foreign_salary.append(
                                round((float(it['job_salary_min']) + float(it['job_salary_max'])) / 2, 2))
                if it['company_type'] is not None and len(it['company_type']) > 0:
                    if "合资" in it['company_type']:
                        if it['job_salary_min'] is not None and len(it['job_salary_min']) > 0 and it['job_salary_max'] is not None and len(it['job_salary_max']) > 0:
                            joint_stock_salary.append(
                                round((float(it['job_salary_min']) + float(it['job_salary_max'])) / 2, 2))
                if it['company_type'] is not None and len(it['company_type']) > 0:
                    if "民营" in it['company_type']:
                        if it['job_salary_min'] is not None and len(it['job_salary_min']) > 0 and it['job_salary_max'] is not None and len(it['job_salary_max']) > 0:
                            private_limited_salary.append(
                                round((float(it['job_salary_min']) + float(it['job_salary_max'])) / 2, 2))
                if it['company_type'] is not None and len(it['company_type']) > 0:
                    if "国企" in it['company_type']:
                        if it['job_salary_min'] is not None and len(it['job_salary_min']) > 0 and it['job_salary_max'] is not None and len(it['job_salary_max']) > 0:
                            nationalized_business_salary.append(
                                round((float(it['job_salary_min']) + float(it['job_salary_max'])) / 2, 2))

            dic['foreign'] = round(float(aver(foreign_salary)),2)
            dic['joint_stock'] = round(float(aver(joint_stock_salary)),2)
            dic['private_limited'] = round(float(aver(private_limited_salary)),2)
            dic['nationalized_business'] = round(float(aver(nationalized_business_salary)),2)
            # 当天为空（没爬）自动填充
            if dic['foreign'] == 0 :
                dic['foreign'] = 11.52
            if dic['joint_stock'] == 0 :
                dic['joint_stock'] = 12.12
            if dic['private_limited'] == 0 :
                dic['private_limited'] = 13.42
            if dic['nationalized_business'] == 0 :
                dic['nationalized_business'] = 12.92
        ...
        sql = "INSERT INTO tb_forecast_companytype_salary (jobtype_two_id,time,result) VALUES (%d,'%s','%s')"
