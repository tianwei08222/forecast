...
class Forecast_Workexper_Jobnum:
    ...
    def get_data(self):
        ...
        # SQL 查询语句
        sql = "select result from tb_forecast_workexper_salary"
        try:
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def to_sql(self):
        ...
        self.result = self.result.replace('\'','')
        cursor = self.db.cursor()
        sql = "update tb_forecast_workexper_salary set forecast = '%s' where id = 1"%(self.result)
        print(sql)
        try:
            cursor.execute(sql)
            self.db.commit()
